...
// -force-revaluate-dangerous-commands and -ask-for-user-approval: their usage
// strings must contain no back-quoted word, because flag.UnquoteUsage turns
// the first back-quoted word into the flag's value name and PrintDefaults
// would then render the boolean flag as taking an argument
// (e.g. "-force-revaluate-dangerous-commands otp_code"),
...
func TestForceRevaluateUsageRendersWithoutValueName(t *testing.T) {
	...
	// -ask-for-user-approval carries the same contract; its usage string is
	// the package-level askForUserApprovalUsage const, single-sourced with the
	// flag registration in main().
