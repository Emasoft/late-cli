...
func (o *BaseOrchestrator) run() {
	...
	// isRunning is cleared on every run-loop exit (terminal-error hang fix).
	// Queued messages are deliberately PRESERVED here: the next run's first
	// turn (onStartTurn) drains them, so a message submitted while an agent
	// was crashing is not silently lost. Explicit Cancel() is the only path
	// that discards queued messages.
