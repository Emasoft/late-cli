...
//     same key set knownModelEntryKeys feeds the strict parser's models[]
//     walk, so a typo'd or missing entry key fails here first;
...
// set of keys one models[] entry accepts — the same set the strict parser's
// knownModelEntryKeys walk validates against.
...
// set the strict parser's knownModelEntryKeys walk enforces: every documented
...
func TestConfigReferenceDocModelsEntryKeysAreValid(t *testing.T) {
	...
	fromParser := knownModelEntryKeys // the strict parser's models[] walk

	for tag := range fromStruct {
		if !fromParser[tag] {
			t.Errorf("ModelSetting json tag %q is missing from knownModelEntryKeys; the strict parser would reject configs using it inside a models entry", tag)
		}
	}
	...
	for lineNo, line := range section {
		...
		key := m[1]
		if !fromStruct[key] {
			t.Errorf("%s:%d: models schema documents entry key %q, which is NOT a ModelSetting json tag — the strict parser would reject it as an unknown entry key", configReferenceDocPath, lineNo, key)
			continue
		}
		if documented[key] {
