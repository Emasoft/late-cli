...
// TestFlexBoolOmitAndTriState pins the JSON surface of the FlexBool fields:
// a plain FlexBool false is omitted (omitempty), an explicit-false
// *FlexBool is kept, and an absent *FlexBool stays nil.
func TestFlexBoolOmitAndTriState(t *testing.T) {
	data, err := json.Marshal(&Config{
		ShowInfoBar: FlexBool(false), // plain false: omitted by omitempty
		UseTools:    flexPtr(false),  // explicit false pointer: kept
	})
	if err != nil {
		t.Fatalf("Marshal() error = %v", err)
	}
	out := string(data)
	if strings.Contains(out, "show-info-bar") {
		t.Fatalf("plain false FlexBool must be omitted, got %s", out)
	}
	if !strings.Contains(out, `"use-tools":false`) {
	...
	var cfg Config
	if err := json.Unmarshal([]byte(`{"use-tools":"off","enable-bash":"y","inject-cwd":1}`), &cfg); err != nil {
		t.Fatalf("Unmarshal() error = %v", err)
	}
	if cfg.UseTools == nil || cfg.UseTools.Bool() {
		t.Fatalf("use-tools \"off\" = %#v, want explicit false", cfg.UseTools)
	}
	if cfg.EnableBash == nil || !cfg.EnableBash.Bool() {
		t.Fatalf("enable-bash \"y\" = %#v, want explicit true", cfg.EnableBash)
	}
	if cfg.InjectCWD == nil || !cfg.InjectCWD.Bool() {
		t.Fatalf("inject-cwd 1 = %#v, want explicit true", cfg.InjectCWD)
	}
	...
	if absent.UseTools != nil {
		t.Fatalf("absent entry = %#v, want nil", absent.UseTools)
	}
}
