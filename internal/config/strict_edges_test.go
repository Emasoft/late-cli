...
func TestParseConfigContent_BOMIsTolerated(t *testing.T) {
	...
	if cfg == nil || cfg.Theme != "late" || cfg.UseTools == nil || !cfg.UseTools.Bool() {
...
func TestParseConfigContent_DuplicateKeyInvalidValueStillErrors(t *testing.T) {
	_, err := parseConfigContent("/x/config.json", []byte(`{"use-tools":"maybe","use-tools":"on"}`))
	if err == nil {
		t.Fatal("expected an error for the invalid boolean synonym in the losing duplicate")
	}
	if !strings.Contains(err.Error(), `"use-tools"`) || !strings.Contains(err.Error(), "not a valid boolean value") {
...
func TestParseConfigContent_CRLFLineEndings(t *testing.T) {
	...
	// `"actve"` starts right after `  "use-tools": ` — 2 spaces + 12 key
	// bytes (with quotes) + ':' + ' ' = column 16, 1-based.
	want := `error in /x/config.json at line 3, column 16: "actve" is not a valid boolean value for "use-tools".`
...
// TestParseConfigContent_NullTriStateFieldsStayNil pins the JSON null
// handling of the tri-state fields: null leaves a *FlexBool nil (unset), so
// the built-in default applies, exactly like an absent entry.
func TestParseConfigContent_NullTriStateFieldsStayNil(t *testing.T) {
	cfg, err := parseConfigContent("/x/config.json", []byte(`{"use-tools": null, "enable-bash": null, "enable-subagents": null, "inject-cwd": null, "show-cwd": null, "show-todo-pane": null, "gemma-thinking": null}`))
	if err != nil {
		t.Fatalf("null tri-state entries must not error: %v", err)
	}
	for name, got := range map[string]*FlexBool{
		"use-tools":        cfg.UseTools,
		"enable-bash":      cfg.EnableBash,
		"enable-subagents": cfg.EnableSubagents,
		"inject-cwd":       cfg.InjectCWD,
		"show-cwd":         cfg.ShowCWD,
		"show-todo-pane":   cfg.ShowTodoPane,
	} {
		if got != nil {
			t.Fatalf("%s: null must leave the field unset (nil), got %#v", name, *got)
		}
	}
	if cfg.GemmaThinking.Bool() {
		t.Fatal("null gemma-thinking must leave the zero value (false)")
	}

	// The resolver contract holds for the null case: unset → default true.
	if got := cfg.ResolveShowTodoPane(); !got {
		t.Fatal("null show-todo-pane must resolve to the open default")
	}
}
...
// PLAIN FlexBool field leaves the zero value without erroring.
func TestConfig_NullPlainFlexBoolKeepsZeroValue(t *testing.T) {
	cfg, err := parseConfigContent("/x/config.json", []byte(`{"show-info-bar": null, "enable-sqz": null}`))
	if err != nil {
		t.Fatalf("null plain-FlexBool entries must not error: %v", err)
	}
	if cfg.ShowInfoBar.Bool() || cfg.EnableSqz.Bool() {
...
// re-accept — and that the reloaded config keeps them unset.
func TestSaveConfig_NilTriStateFieldsAreOmitted(t *testing.T) {
	...
	for _, field := range []string{"use-tools", "enable-bash", "enable-subagents", "inject-cwd", "show-cwd", "show-todo-pane"} {
		if strings.Contains(string(data), `"`+field+`"`) {
			t.Fatalf("nil tri-state %s must be omitted from the saved config, got %s", field, data)
		}
	}

	// End-to-end: a config with a synonym value and several unset tri-state
	// fields saves canonically and reloads under the strict parser with the
	// tri-state preserved.
	configRoot := t.TempDir()
	setUserConfigEnv(t, configRoot)
	...
	loaded.ShowTodoPane = flexPtr(false)
	loaded.EnableSqz = FlexBool(true)
	...
	if !strings.Contains(string(raw), `"show-todo-pane": false`) {
		t.Fatalf("saved config %s must keep the explicit false", raw)
	}
	...
	if reloaded.ShowTodoPane == nil || reloaded.ShowTodoPane.Bool() {
		t.Fatal("reloaded show-todo-pane must stay an explicit false")
	}
	if reloaded.UseTools != nil {
		t.Fatal("reloaded use-tools must stay unset")
	}
	if !reloaded.EnableSqz.Bool() {
		t.Fatal("reloaded enable-sqz must be true")
	}
}
