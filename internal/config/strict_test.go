...
func TestParseConfigContent_TrailingCommaLineCol(t *testing.T) {
	content := "{\n" +
		...
		"  \"use-tools\": \"on\",\n" +
		"}"
	// encoding/json's scanner flags the trailing comma itself: the error
	// position is exactly that comma (end of line 3).
	_, err := parseConfigContent("/x/config.json", []byte(content))
	if err == nil {
		t.Fatal("expected a syntax error for the trailing comma")
	}
	want := `error in /x/config.json at line 3, column 21: invalid character ',' looking for beginning of value`
	if err.Error() != want {
		t.Fatalf("error = %q\nwant  %q", err.Error(), want)
	}
}
...
func TestParseConfigContent_UnknownKeyExactR3Message(t *testing.T) {
	...
	want := `error in /Users/u/Library/Application Support/late/config.json at line 2, column 3: "compaction_mode" is not a valid config.json entry. Did you mean "compaction-mode"?`
	if err.Error() != want {
		t.Fatalf("error = %q\nwant  %q", err.Error(), want)
	}
}
...
func TestParseConfigContent_UnknownKeySuggestionDistance(t *testing.T) {
	cases := []struct {
		...
		{key: "use-tool", wantSugges: "use-tools"},
		{key: "compaction_mod", wantSugges: "compaction-mode"},
		{key: "SystemPrompt", wantSugges: "system-prompt"},
...
// TestParseConfigContent_EnumValueError pins R3 for multi-value entries:
// the wrong VALUE is reported at the value's exact offset with a suggestion.
func TestParseConfigContent_EnumValueError(t *testing.T) {
	content := "{\n  \"compaction-mode\": \"shado\",\n  \"theme\": \"late\"\n}"
	_, err := parseConfigContent("/x/config.json", []byte(content))
	if err == nil {
		t.Fatal("expected an invalid compaction-mode error")
	}
	want := `error in /x/config.json at line ` + strconv.Itoa(lineOf(content, `"shado"`)) +
		`, column ` + strconv.Itoa(runeColumn(content, `"shado"`)) +
		`: "shado" is not a valid compaction-mode value. Did you mean "shadow"?`
	if err.Error() != want {
		t.Fatalf("error = %q\nwant  %q", err.Error(), want)
	}
}
...
// the three known values parse; an unknown one errors at the value offset.
func TestParseConfigContent_PermissionModeEnum(t *testing.T) {
	for _, mode := range []string{PermissionModeAskForUserApproval, PermissionModeUnsupervised, PermissionModeForceRevaluate} {
...
// TestParseConfigContent_CompactionBackendFreeForm pins the free-form
// compaction-backend validation: "offline" and any non-empty provider name
// parse; only a blank-after-trim value is rejected.
func TestParseConfigContent_CompactionBackendFreeForm(t *testing.T) {
	for _, backend := range []string{"offline", "typesafe", "system-one", "x"} {
		if _, err := parseConfigContent("/x/config.json", []byte(`{"compaction-backend":"`+backend+`"}`)); err != nil {
			t.Fatalf("compaction-backend %q: unexpected error %v", backend, err)
		}
	}
	_, err := parseConfigContent("/x/config.json", []byte(`{"compaction-backend":"   "}`))
	if err == nil {
		t.Fatal("expected an error for a blank compaction-backend value")
	}
	if !strings.Contains(err.Error(), "is not a valid compaction-backend value") {
		t.Fatalf("error = %q, want the compaction-backend message", err.Error())
	}
}
...
func TestParseConfigContent_TypeErrorLineCol(t *testing.T) {
	content := "{\n  \"theme\": \"late\",\n  \"compaction-threshold-percent\": \"many\"\n}"
	_, err := parseConfigContent("/x/config.json", []byte(content))
	if err == nil {
		t.Fatal("expected a type error")
	}
	want := `error in /x/config.json at line ` + strconv.Itoa(lineOf(content, `"many"`)) +
		`, column ` + strconv.Itoa(runeColumn(content, `"many"`)) +
		`: "compaction-threshold-percent" must be a number, found a string`
	if err.Error() != want {
		t.Fatalf("error = %q\nwant  %q", err.Error(), want)
	}
}
...
func TestParseConfigContent_WrongTypedBoolean(t *testing.T) {
	content := `{"enable-sqz": [1, 2]}`
	_, err := parseConfigContent("/x/config.json", []byte(content))
	if err == nil {
		t.Fatal("expected an invalid boolean value error")
	}
	if !strings.Contains(err.Error(), `"enable-sqz"`) || !strings.Contains(err.Error(), "Accepted values are:") {
...
func TestParseConfigContent_FlexBoolSynonymErrorPositioned(t *testing.T) {
	...
	want := `error in /x/config.json at line ` + strconv.Itoa(lineOf(content, `"actve"`)) +
		...
		`: "actve" is not a valid boolean value for "use-tools". Accepted values are: ` +
...
func TestParseConfigContent_AllKeysWithSynonymsRoundTrip(t *testing.T) {
  ...
  "show-info-bar": "Activated",
  "show-timestamps": "Disable",
  "compaction-threshold-percent": 65,
  "compaction-mode": "enabled",
  "jev-autocompact": "on",
  "jev-autocompact-percent": 90,
  "compaction-max-elide-percent": 60,
  "compaction-threshold": 0.4,
  "compaction-protected-floor": 10,
  "compaction-backend": "offline",
  "compaction-retrieval": "yes"
}`
	...
	// Spot-check the synonym conversions.
	if cfg.UseTools == nil || cfg.UseTools.Bool() {
		t.Fatalf(`use-tools "no" = %#v, want explicit false`, cfg.UseTools)
	}
	if cfg.ShowTodoPane == nil || cfg.ShowTodoPane.Bool() {
		t.Fatalf(`show-todo-pane "NOT" = %#v, want explicit false`, cfg.ShowTodoPane)
	}
	if !cfg.ShowInfoBar.Bool() {
		t.Fatalf(`show-info-bar "Activated" = %v, want true`, cfg.ShowInfoBar.Bool())
	}
	if cfg.ShowTimestamps.Bool() {
		t.Fatalf(`show-timestamps "Disable" = %v, want explicit false`, cfg.ShowTimestamps.Bool())
	}
	if !cfg.GemmaThinking.Bool() || !cfg.EnableSqz.Bool() {
		t.Fatal("gemma-thinking/enable-sqz synonyms did not parse as true")
	}
	...
	// The plain FlexBool false (show-timestamps "Disable") is omitted by
	// omitempty, exactly like the old plain-bool behavior.
	if _, present := canonical["show-timestamps"]; present {
		t.Fatalf("re-marshaled config must omit the zero-valued show-timestamps entry, got %s", data)
	}
...
func TestSaveConfig_WritesPlainBooleans(t *testing.T) {
	...
	cfg.ShowInfoBar = FlexBool(true)
	cfg.ShowTimestamps = FlexBool(false)
	cfg.UseTools = flexPtr(false)
	cfg.JevAutocompact = FlexBool(true)
	...
	out := string(raw)
	for _, want := range []string{`"show-info-bar": true`, `"use-tools": false`, `"jev-autocompact": true`} {
		if !strings.Contains(out, want) {
			t.Fatalf("saved config %s does not contain %q", out, want)
		}
	}
	// The plain FlexBool false is omitted by omitempty (same as the old
	// plain-bool behavior) — the pointer-typed tri-state keeps its false.
	if strings.Contains(out, "show-timestamps") {
		t.Fatalf("saved config %s must omit the zero-valued show-timestamps entry", out)
	}
	reloaded, err := LoadConfig()
	if err != nil {
		t.Fatalf("strict reload of the saved config failed: %v", err)
	}
	if !reloaded.ShowInfoBar.Bool() || reloaded.ShowTimestamps.Bool() || !reloaded.JevAutocompact.Bool() {
		t.Fatal("reloaded booleans do not match the saved values")
	}
	if reloaded.UseTools == nil || reloaded.UseTools.Bool() {
		t.Fatal("reloaded use-tools must be an explicit false")
	}
}
